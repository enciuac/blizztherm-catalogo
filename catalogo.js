...
function urlConUtm(baseUrl, campaign) {
  const u = new URL(baseUrl);
  u.searchParams.set("utm_source", "catalogo-interno");
...
function urlCompraBlizzcool(p) {
  const base = URLS_BLIZZTHERM[p.id];
  return base ? urlConUtm(base, p.id) : null;
}
...
function renderPrivacidad() {
  ...
  document.documentElement.lang = lang();
  document.title = `${t("privacidadTitle")} · BlizzTherm`;
